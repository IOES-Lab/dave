...
extern "C"
{
  void * sonar_wgpu_create(
    ...
    float h_fov, float v_fov, uint64_t seed);

  float * sonar_wgpu_compute(
    void * engine,
    const float * depth, const float * normals,
    const float * refl,  const float * beam_corr,
    uint32_t n_beams, uint32_t n_rays, uint32_t n_freq,
    uint64_t frame, float beam_corr_sum);
    ...
    SonarComputeOutput & output) override
  {
    ...
    for (int b = 0; b < nb; ++b)
    {
      for (int ray = 0; ray < nr; ++ray)
      {
        ...
        const float TL   = std::exp(
          -static_cast<float>(input.attenuation) * r) / r;
  ...
  SonarComputeOutput & output)
{
  ...
  if (!engine_ || engNb_ != nb || engNr_ != nr || engNf_ != nf) {
    ...
    engine_ = sonar_wgpu_create(
      ...
      static_cast<float>(input.soundSpeed),
      static_cast<float>(input.bandwidth),
      static_cast<float>(input.maxDistance),
      static_cast<float>(input.attenuation),
      static_cast<float>(input.sourceLevel),
      input.sensorGain,
      input.hFOV,
      input.vFOV,
      input.seed);
